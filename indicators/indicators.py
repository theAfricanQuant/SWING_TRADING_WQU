...
def moving_average(df, n):
    ...
    """
    MA = pd.Series(
        df['Close'].rolling(n, min_periods=n).mean(), name=f'MA_{str(n)}'
    )
    df = df.join(MA)
    return df
...
def exponential_moving_average(df, n):
    ...
    """
    EMA = pd.Series(
        df['Close'].ewm(span=n, min_periods=n).mean(), name=f'EMA_{str(n)}'
    )
    df = df.join(EMA)
    return df
...
def average_true_range(df, n):
    ...
    while i < len(df.index)-1:
        ...
        i += 1
    TR_s = pd.Series(TR_l)
    ATR = pd.Series(TR_s.ewm(span=n, min_periods=n).mean(), name=f'ATR_{n}')
    df = df.join(ATR)
    return df


def bollinger_bands(df, n , std_value):
    """
    ...
    b3 =  MA + std_value * MSD
    b4 = MA - std_value * MSD
    B3 = pd.Series(b3, name=f'BBUpper{str(n)}')
    B4 = pd.Series(b4, name=f'BBLower{str(n)}')
    df = df.join(B3)
    df = df.join(B4)
    return df
...
def commodity_channel_index(df, n):
    ...
    CCI = pd.Series(
        ...
        / PP.rolling(n, min_periods=n).std(),
        name=f'CCI_{str(n)}',
    )
    df = df.join(CCI)
    return df
...
def coppock_curve(df, n):
    ...
    ROC2 = M / N
    Copp = pd.Series(
        (ROC1 + ROC2).ewm(span=n, min_periods=n).mean(), name=f'Copp_{str(n)}'
    )
    df = df.join(Copp)
    return df
...
def standard_deviation(df, n):
    ...
    df = df.join(
        pd.Series(
            df['Close'].rolling(n, min_periods=n).std(), name=f'STD_{str(n)}'
        )
    )
    return df